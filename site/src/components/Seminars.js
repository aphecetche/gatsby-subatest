    ...
    <StaticQuery
      ...
      render={data => (
        data.allMdx.edges.map(({ node }, index) => {
          if((moment(node.frontmatter.date,"YYYY-MM-DD HH:mm:ss Z").isAfter(earlydate)&&moment(node.frontmatter.date,"YYYY-MM-DD HH:mm:ss Z").isBefore(latedate)))
          ...
          //<td>{earlydate}</td> 
         return <li>{moment(node.frontmatter.date,"YYYY-MM-DD HH:mm:ss Z").format("DD-MM-YYYY")}, &nbsp;
         {t(node.frontmatter.type)}:&nbsp;
           {/* <a href={node.fields.slug}>{node.frontmatter.title}</a> */}
           <Link to={localizeUrl(node.fields.slug,currentLanguage)}>{node.frontmatter.title}
           {(node.frontmatter.title!==""&&node.frontmatter.title2!=="") ?  " & " : ""}{node.frontmatter.title2}</Link></li>
         return ""
        })
      )}
      />
    </ul> 
    <p><Link to="allseminars">{plus}...</Link></p></div>
